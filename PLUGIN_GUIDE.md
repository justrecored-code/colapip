...
插件代码托管在独立的 git 仓库中。平台通过 `config/plugins.json` 清单发现可用插件：

```json
[
  { "name": "my-plugin", "repo": "https://github.com/xxx/colapip-my-plugin" }
]
```

启动时平台检查 `plugins/<name>/` 是否存在——不存在就自动 `git clone`，存在就直接加载。用户拿到平台后，把想用的插件加到 `plugins.json` 里就行，不用手动 clone。

也可以直接手动把插件目录放到 `plugins/` 下（git clone 或拷贝），平台照常加载。`plugins.json` 只在目录缺失时才用。
...
`cliDeps` — npm 包名数组。插件依赖的外部 CLI 工具。平台加载插件时自动检测，不在 PATH 则 `npm install -g` 自动安装。安装失败跳过该插件。
