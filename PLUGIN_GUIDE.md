...
插件代码托管在独立的 git 仓库中（命名规则 `colapip-<name>`）。`platform.json` 中配置一个或多个 GitHub 用户名：

```json
{ "pluginRegistry": "your-github-username" }
// 或多源:
{ "pluginRegistry": ["user-a", "user-b", "https://github.com/user-c"] }
```

启动时平台自动从 GitHub API 发现 `colapip-*` 仓库，目录缺失则自动 `git clone`。

也可以直接手动把插件目录放到 `plugins/` 下（git clone 或拷贝），平台照常加载。
